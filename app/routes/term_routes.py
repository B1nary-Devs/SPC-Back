...
terms_collection = mongo.db.terms # colecao de termos do mongo db
...
def create_term():
    try:
        ...
        versionTerm = 1.0
        termExists = terms_collection.find_one({'nome_termo': nome_termo})
        if termExists:
            versionTerm = termExists['versao'] + 0.1
        
        dataTerm = {
            'nome_termo': nome_termo,
            'descricao': data['descricao'],
            'prioridade': data['prioridade'],
            'data_cadastro': datetime.timestamp(),
            'versao': versionTerm
        }

        result = terms_collection.insert_one(dataTerm)
        if result.matched_count == 0:
            return jsonify({'error': 'Termo e Condicoes não inserido!'}), 404
