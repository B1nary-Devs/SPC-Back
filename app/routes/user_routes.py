...
users_collection = mongo.db.users #colecao de usuarios do mongo db
...
def create_user():
    try:
        ...
        result = users_collection.insert_one(dataUser)
        if result.matched_count == 0:
            return jsonify({'error': 'Usuário não inserido!'}), 404
        
        return jsonify({'message': 'Usuário criado com sucesso!', 'user_id': str(result.inserted_id), **result }), 201
...
def updateUser(usuario_cpf_cnpj):
    try:
        ...
        if 'endereço' in data:
            update_fields['endereço'] = data['endereço']
        if 'senha' in data:
            password_hash = generate_password_hash(data['senha'])
